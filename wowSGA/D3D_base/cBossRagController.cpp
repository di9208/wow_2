...
void cBossRagController::SetUp(std::vector<tagMon> Monster)
{
	...
	D3DXMatrixScaling(&matS, 0.0045f, 0.0045f, 0.0045f);
	D3DXMatrixTranslation(&matT, 0.0f, 0, 0.0f);
...
{
	D3DXMATRIXA16 matR2, matS2, matT2, matX2,matY, World2;
	D3DXMatrixScaling(&matS2, 0.025f, 0.025f, 0.025f);

	D3DXMatrixRotationX(&matX2, D3DX_PI / -2.f);
	//D3DXMatrixRotationY(&matR2, m_fBossRotY);

	//D3DXMatrixIdentity(&matT2);
	
	//D3DXMatrixRotationY(&matX2, D3DX_PI / -2.0f);
	D3DXMatrixRotationY(&matY, D3DX_PI / -2.f);
	D3DXVECTOR3 dir2;
	//D3DXVec3TransformNormal(&dir2, &m_vBossDir, &matR2);
	D3DXVECTOR3 vUp2(0, 1, 0);
	D3DXVECTOR3 vRight2;
	D3DXVec3Cross(&vRight2, &vUp2, &m_vBossDir);
	D3DXVec3Normalize(&vRight2, &vRight2);
	D3DXVec3Cross(&vUp2, &m_vBossDir, &vRight2);
	D3DXMatrixLookAtLH(&matR2, &D3DXVECTOR3(0, 0, 0), &m_vBossDir, &vUp2);
	D3DXMatrixTranspose(&matR2, &matR2);

	D3DXMatrixTranslation(&matT2, m_vBossPos.x, m_vBossPos.y, m_vBossPos.z);
	World2 = matS2 * matX2* matY* matR2 * matT2;


	m_obbw = matX2* matY* matR2 *matT2;
	if (m_pSkinnedMesh_rag)
...
void cBossRagController::skillRender()
{
	...
	D3DXMatrixTranslation(&matT, m_vBossPos.x, m_vBossPos.y + 0.1f, m_vBossPos.z);
