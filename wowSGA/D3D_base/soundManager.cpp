...
//사운드 추가
void soundManager::addSound(std::string keyName, std::string soundName, bool background, bool loop)
{
	//돌림노래냐?
	if (loop)
	{
	...
	//맵에 사운드를 키값과 함께 넣어준다
	_mTotalSounds.insert(std::make_pair(keyName, &_sound[_mTotalSounds.size()]));
}
